...
<input type="hidden" id="NVIDIA_GRAFANA_HOST" value="${dataListWrapper.grafanaHost}">
    ...
    <%-- <nav class="navbar-default navbar-static-side" role="navigation">
        <div class="sidebar-collapse">
            <ul class="nav metismenu" id="side-menu">
                <li class="nav-header logo" style="border-bottom: 1px solid #5A5A5A;">  <img alt="image"  src="../resources/core/img/logo.png" width="200"/>
               ...
               <c:forEach items="${dataListWrapper.clusterDataList}" var="clusterData" varStatus="status">
                ...
                <li class="active">
                    <a href="<c:url value="/nvidia"/>"><i class="fa fa-th-large"></i> <span class="nav-label">${clusterData.clusterId}</span><span class="fa arrow"></span></a>
						<ul class="nav nav-second-level">
						<c:forEach items="${dataListWrapper.nodeInfoDataList}" var="nodeInfoDatax" varStatus="status">
						
						<c:if test="${nodeInfoDatax.clusterId==clusterData.clusterId}">
						
                        <c:choose>
						<c:when test="${nodeInfoDatax.name==nodeInfoData.name}">
						<li class="active">
                             <a href="#">${nodeInfoDatax.name}</a>
                        </li>
						</c:when>
						<c:otherwise>
						<li>
                             <a href="<c:url value="/node/${nodeInfoDatax.serialId}&${nodeInfoDatax.clusterId}"/>">${nodeInfoDatax.name}</a>
                        </li>
						</c:otherwise>
						</c:choose>
                        </c:if>
				...
				<!--	
                <li>
                    <a href="#"><i class="fa fa-list"></i> <span class="nav-label">Jobs</span></a>
                </li>
                <li>
                    <a href="#"><i class="fa fa-exclamation"></i> <span class="nav-label">Alerts</span></a>
                </li>
                <li>
                    <a href="#"><i class="fa fa-gear"></i> <span class="nav-label">Settings</span></a>
                </li>
				-->
            </ul>
        </div>
    ...
    <jsp:include page="menu.jsp"></jsp:include>
        ...
        <div class="navbar-header">
            <a class="navbar-minimalize minimalize-styl-2 btn btn-primary " href="#" style="padding: 7px 12px;"><i class="fa fa-bars"></i> </a>
            <a class="minimalize-styl-2 btn btn-primary " href="/nvidia/nvidia" style="padding: 7px 12px;"><i class="fa fa-home"></i> </a>
        </div>
            <ul class="nav navbar-top-links navbar-right">
                <li>
                    <span class="m-r-sm text-muted welcome-message">Welcome David Williams.</span>
                </li>
			...
			<div class="wrapper wrapper-content">
				<div class="row">
					<!-- <div class="col-lg-12">
						<h2 style="float: left; margin-top: 0px;">
							<a href="<c:url value="/nvidia"/>"><i
								class="fa fa-home fa-1x" style="color: #1ab394;"></i>&nbsp;&nbsp;${clusterData.clusterId}
							</a>: ${nodeInfoData.name}
						</h2>
					</div>-->
                ...
                <div class="col-lg-6">
					<div class="ibox float-e-margins">
							<div class="ibox-title">
								<h5>
									<i class="fa fa-share-alt"></i> Node Info
								</h5>
								<div class="ibox-tools">
							...
							<div class="ibox-content">
								<ul class="todo-list m-t small-list">
									<li><span>Key : ${nodeInfoData.nodeKey} </span></li>
									<input type="hidden" id="node_licensekey"
										value="${nodeInfoData.serialId}">
									<li><span>Node Serial Id : ${nodeInfoData.serialId}
									</span></li>
									<li><span>Node host Name : ${nodeInfoData.name} </span></li>
									<li><span><center>
												<b> Node Network Information </b>
											</center></span></li>
									<li><span>Subnet : ${nodeInfoData.subnet} </span></li>
									<li><span>Gateway : ${nodeInfoData.gateway} </span></li>
									<li><span>Mode : ${nodeInfoData.mode} </span></li>
									<li><span>IP Address : ${nodeInfoData.ipAddress} </span></li>
									<li><span>SW version : ${nodeInfoData.swVersion} </span></li>
									<li><span>FW version : ${nodeInfoData.fwVersion} </span></li>
									<li><span> Created Time :
											${nodeInfoData.createdTime} </span></li>
									<li><span> Status : <c:if
												test="${nodeInfoData.status == 'Connected'}">
												<small  class="label label-primary">${nodeInfoData.status}</small>
											</c:if> <c:if test="${nodeInfoData.status == 'Disconnected'}">
												<button type="button" class="btn btn-warning btn-xs">${nodeInfoData.status}</button>
											</c:if> <c:if test="${nodeInfoData.status == 'NA'}">
												<button type="button" class="btn btn-danger btn-xs">${nodeInfoData.status}</button>
											</c:if>
									</span> 
									</li>
									<li><span><center>
												<b> Node firmware version </b>
											</center></span></li>
									<li><span> IPMI : ${nodeInfoData.ipmi} </span></li>
									<li><span>BIOS VERSION :
											${nodeInfoData.biosVersion} </span></li>
								</ul>
							</div>
						</div>
						<div class="ibox float-e-margins">
						...
						<div class="ibox-content table-responsive">
							<table class="table table-hover no-margins">
								...
								<tbody>
									...
									<c:forEach items="${dataListWrapper.jobTaskDataList}" var="jobTaskData"
										varStatus="status">
										<tr>
											<%-- <td>${task.id}</td> --%>
								...
								<li><span>State : ${jobTaskData.state} </span></li></c:if>"><i class="fa fa-eye primary" title="Details"></i></a>
												 ...
												 <a class="btn job_details" style="padding: 0px 0px;" href="${jobTaskData.appId}"><i class="fa fa-times-circle fa-1x text-danger"></i></a>
											</td>
				...
				<div class="col-lg-6">
					...
					<div class="ibox float-e-margins" id="alert_tab_data">
							...
							<div class="ibox-content">
								...
								<table class="footablex table table-stripped" data-page-size="8" data-filter=#filter>
									<thead>
										<tr>
											<th>Name</th>
											<th>Node</th>
												...
												<c:forEach items="${dataListWrapper.childList}" var="child"
													...
													<tr class="gradeX">
														...
														<td class="center"><a style="cursor: pointer;"
															data-toggle="modal" data-target="#alert" data-disp="" data-alertstatusid ="${child.getAlertStatus()}" 
															data-notificationid="${child.getAlertKey()}"> <i class="fa fa-eye primary" title="Details"></i></a>
														</td>
    ...
    <!-- Flot -->
    <script src="../resources/core/js/plugins/flot/jquery.flot.js"></script>
    <script src="../resources/core/js/plugins/flot/jquery.flot.tooltip.min.js"></script>
    <script src="../resources/core/js/plugins/flot/jquery.flot.spline.js"></script>
    <script src="../resources/core/js/plugins/flot/jquery.flot.resize.js"></script>
    <script src="../resources/core/js/plugins/flot/jquery.flot.pie.js"></script>
    <script src="../resources/core/js/plugins/flot/jquery.flot.symbol.js"></script>
    <script src="../resources/core/js/plugins/flot/jquery.flot.time.js"></script>

    <!-- Peity -->
    <script src="../resources/core/js/plugins/peity/jquery.peity.min.js"></script>
    <script src="../resources/core/js/demo/peity-demo.js"></script>
    ...
    <!-- Jvectormap -->
    <script src="../resources/core/js/plugins/jvectormap/jquery-jvectormap-2.0.2.min.js"></script>
    <script src="../resources/core/js/plugins/jvectormap/jquery-jvectormap-world-mill-en.js"></script>

    <!-- EayPIE -->
    <script src="../resources/core/js/plugins/easypiechart/jquery.easypiechart.js"></script>

    <!-- Sparkline -->
    <script src="../resources/core/js/plugins/sparkline/jquery.sparkline.min.js"></script>

 <!-- Morris -->
    <script src="../resources/core/js/plugins/morris/raphael-2.1.0.min.js"></script>
    <script src="../resources/core/js/plugins/morris/morris.js"></script>
    <!-- Sparkline demo data  -->
    <script src="../resources/core/js/demo/sparkline-demo.js"></script>
	...
	<script>
		$(document).ready(function() {
			...
			$('.footablex').footable();
			$('.footablex').trigger('footable_filter', {filter: "NeedAck"});
	...
	var NVIDIA_HOST = $('#NVIDIA_HOST').val();
		var cur_dat;
		var pre_dat;
		var idofnode = $("#node_licensekey").val();
		//alert(idofnode);
		var dataGetter_cur;
		var dataGetter_pre;
		if (idofnode === 'node1') {
			dataGetter_cur = node1_cur;
			dataGetter_pre = node1_pre;
		} else if (idofnode === 'node2') {
			dataGetter_cur = node2_cur;
			dataGetter_pre = node2_pre;

		} else {
			console.log(" Something is wrong with the nodes. Please report to admin");
		}
		...
		jQuery(document).ready(	function() {
			...
			$('#containerx').on('show.bs.modal', function(e) {
				...
				$(".intervalCancel").on("click", function(){
					...
					$("#frame_change1").attr("src", dataGetter_cur.one);
					$("#frame_change2").attr("src", dataGetter_cur.two);
					$("#frame_change3").attr("src", dataGetter_cur.three);
					$("#frame_change4").attr("src", dataGetter_cur.four);
					...
					//cur_dat = day + ' ' + monthNames[monthIndex] + ' ' + year;
					//pre_dat = pre_day+ ' ' +monthNames[pre_month]+ ' ' + pre_year;
					cur_dat = monthNames[monthIndex] + ' ' + day + ' ' + year;
					pre_dat = monthNames[pre_month] + ' ' + pre_day + ' '
							+ pre_year;

					var selected_tf = document
							.getElementById("selected_timeframe");
	...
	var selected_tf = document.getElementById("selected_timeframe");
	 ...
	 }else if(change_id=="2")
	 {
	    ...
	    $("#frame_change2").attr("src",dataGetter_pre.one);
		$("#frame_change1").attr("src",dataGetter_pre.two);
		$("#frame_change3").attr("src",dataGetter_pre.three);
		$("#frame_change4").attr("src",dataGetter_pre.four);
	 }else
	 {
	...
	$('#start12').click(function() {
		$('#loadingmessage').show();
		$.post('http://ec2-52-27-102-209.us-west-2.compute.amazonaws.com/nvidia/digits', function(data) {
			$('#after_ajax').html(data);
			$('#loadingmessage').hide();
		});
